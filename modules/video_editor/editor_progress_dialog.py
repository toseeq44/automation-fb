"""
modules/video_editor/editor_progress_dialog.py
Editor Progress Dialog - Shows real-time progress during batch processing
"""
...
from modules.video_editor.editor_batch_processor import EditorBatchWorker, BatchProcessor

logger = get_logger(__name__)


class EditorProgressDialog(QDialog):
    """
    Progress dialog for batch video processing
    ...
    def create_header(self) -> QFrame:
        ...
        # Title
        title = QLabel("Processing Videos...")
    ...
    def on_video_completed(self, result: dict):
        ...
        if status == 'success':
            ...
            if result.get('deleted'):
                deleted = int(self.get_stat_value("deleted") or "0") + 1
                self.update_stat("deleted", str(deleted))
        elif status == 'failed':
    ...
    def on_processing_finished(self, summary: dict):
        ...
        self.log_message("BATCH PROCESSING COMPLETE", "success")
        ...
        self.log_message(f"Sources deleted: {summary['sources_deleted']}", "info")
        ...
        if summary['was_cancelled']:
            ...
            QMessageBox.information(
                ...
                f"Total: {summary['successful']} videos"
            )
    ...
    def open_output_folder(self):
        ...
        if self.config.get('mapping'):
            dest_folder = self.config['mapping'].destination_folder
            if os.path.exists(dest_folder):
                ...
                if sys.platform == 'win32':
                    ...
                    subprocess.run(['open', dest_folder])
                else:
                    subprocess.run(['xdg-open', dest_folder])
